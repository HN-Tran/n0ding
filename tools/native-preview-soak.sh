...
mode=gate; duration=259200; sample_interval=300; restart_interval=21600
...
usage() { cat <<'EOF'
Usage: sudo tools/native-preview-soak.sh [options]
...
Options:  --service NAME --url URL --duration SEC --sample-interval SEC
          --restart-interval SEC --min-free-percent N --min-free-bytes N
...
Defaults are a 72-hour run, five-minute samples, six-hour planned restarts,
and a 10% critical free-space floor. The runner observes a native systemd
deployment; it never installs software, changes configuration, or deletes data.
EOF
}
die() { printf 'native-preview-soak: %s\n' "$*" >&2; exit 1; }
while (($#)); do case "$1" in
  --mode) mode=${2:?}; shift 2;;
  ...
  --url) base_url=${2%/}; shift 2;; --duration) duration=${2:?}; shift 2;;
  --sample-interval) sample_interval=${2:?}; shift 2;; --restart-interval) restart_interval=${2:?}; shift 2;;
...
[[ $duration =~ ^[0-9]+$ && $duration -ge 60 ]] || die "duration must be an integer >= 60"
if [[ $mode == gate ]]; then
  ((duration>=259200)) || die "a qualifying gate must run at least 259200 seconds"
  [[ -n $workload_hook && -n $canary_file && -n $anchor_hook ]] || die "gate mode requires workload, canary, and off-host anchor hooks"
...
[[ $sample_interval =~ ^[0-9]+$ && $sample_interval -gt 0 ]] || die "sample interval must be positive"
[[ $restart_interval =~ ^[0-9]+$ && $restart_interval -gt 0 ]] || die "restart interval must be positive"
...
for cmd in curl find grep journalctl python3 sha256sum systemctl; do command -v "$cmd" >/dev/null || die "missing command: $cmd"; done
...
started_epoch=$(date +%s); started_utc=$(date -u +%FT%TZ); deadline=$((started_epoch+duration))
next_restart=$((started_epoch+restart_interval)); planned_restarts=0; samples=0; failure=
...
write_progress() {
  ...
  python3 - "$tmp" "$started_epoch" "$now" "$deadline" "$samples" "$planned_restarts" "$failure" <<'PY'
import json,sys
p,s,n,d,sm,r,f=sys.argv[1:]
with open(p,"w") as out:
 json.dump({"start_epoch":int(s),"now_epoch":int(n),"deadline_epoch":int(d),"elapsed_seconds":int(n)-int(s),"samples":int(sm),"planned_restarts":int(r),"failure":f or None},out,indent=2); out.write("\n")
PY
  mv "$tmp" "$evidence_root/progress.json"
}
validate_workload() {
  [[ -r $evidence_root/workload-summary.json ]] || return 1
  python3 - "$evidence_root/workload-summary.json" <<'PY'
import json,sys
d=json.load(open(sys.argv[1]))
for name in ("npm","pip","uv","oci"):
 e=d["ecosystems"][name]
 for phase in ("cold","warm","post_restart"):
  assert isinstance(e[phase],int) and e[phase] >= 1
...
finish() {
  ...
  if ((rc != 0)) || [[ -n $failure ]] || ((elapsed < duration)); then result=fail; fi
  if [[ $result == pass && $mode == gate ]] && ! validate_workload; then failure="workload summary is missing or incomplete"; result=fail; fi
  if [[ $result == pass ]]; then
...
PY
  ...
  python3 - "$evidence_root/result.json" "$result" "$mode" "$started_utc" "$(date -u +%FT%TZ)" "$duration" "$elapsed" "$samples" "$planned_restarts" "$expected_version" "$binary_hash" "$config_hash" "$effective_unit_hash" "$workload_hash" "$canary_definitions_hash" "$anchor_hash" "$failure" <<'PY'
import json,sys
p,result,mode,start,end,requested,elapsed,samples,restarts,version,bh,ch,uh,wh,canh,ah,failure=sys.argv[1:]
with open(p,"w") as out:
 json.dump({"result":result,"mode":mode,"qualifying_gate":result=="pass" and mode=="gate","started_utc":start,"ended_utc":end,"requested_seconds":int(requested),"elapsed_seconds":int(elapsed),"duration_completed":int(elapsed)>=int(requested),"samples":int(samples),"planned_restarts":int(restarts),"expected_version":version,"binary_sha256":bh,"config_sha256":ch,"effective_unit_sha256":uh,"workload_hook_sha256":wh,"canary_definitions_sha256":canh,"anchor_hook_sha256":ah,"failure":failure or None},out,indent=2); out.write("\n")
PY
  if [[ $result == fail ]]; then
...
sample() {
  local now stamp dir status version free_pct free_bytes cache_bytes cache_growth pid rss fds temps current unit_now nrestarts unit_effective_now
  now=$(date +%s); printf -v stamp '%s-%06d' "$(date -u +%Y%m%dT%H%M%SZ)" "$samples"; dir=$evidence_root/snapshots/$stamp; mkdir "$dir"
  systemctl is-active --quiet "$service" || fail_run "service is not active"
  ...
  journalctl -u "$service" --since "@$((now-sample_interval-5))" --until "@$now" --no-pager >"$dir/journal.log" || true
  ...
  check_canaries; [[ -z $workload_hook ]] || "$workload_hook" "$base_url" "$dir" "$evidence_root" || fail_run "workload hook failed"
  samples=$((samples+1)); write_progress
}

python3 - "$evidence_root/run.json" "$mode" "$started_utc" "$duration" "$sample_interval" "$restart_interval" "$service" "$base_url" "$expected_version" "$binary_hash" "$config_hash" "$effective_unit_hash" "$workload_hash" "$canary_definitions_hash" "$anchor_hash" "$min_free_percent" "$min_free_bytes" "${max_rss_kib:-null}" "${max_fds:-null}" "${max_cache_growth_bytes:-null}" "${max_temp_files:-null}" <<'PY'
import json,sys
p,mode,start,duration,sample,restart,service,url,version,bh,ch,uh,wh,canh,ah,freepct,freebytes,rss,fds,growth,temps=sys.argv[1:]
num=lambda x: None if x=="null" else int(x)
with open(p,"w") as out:
 json.dump({"mode":mode,"started_utc":start,"duration_seconds":int(duration),"sample_interval_seconds":int(sample),"restart_interval_seconds":int(restart),"service":service,"base_url":url,"expected_version":version,"binary_sha256":bh,"config_sha256":ch,"effective_unit_sha256":uh,"workload_hook_sha256":wh,"canary_definitions_sha256":canh,"anchor_hook_sha256":ah,"minimum_free_percent":int(freepct),"minimum_free_bytes":int(freebytes),"bounds":{"max_rss_kib":num(rss),"max_fds":num(fds),"max_cache_growth_bytes":num(growth),"max_temp_files":num(temps)}},out,indent=2); out.write("\n")
PY
sample
while (($(date +%s)<deadline)); do
  now=$(date +%s)
  remaining=$((deadline-now)); sleep_for=$sample_interval; ((remaining<sleep_for)) && sleep_for=$remaining; sleep "$sleep_for"; sample
  now=$(date +%s)
  if ((now>=next_restart)); then
    ...
    printf '%s\tplanned\t%s\t%s\t%s\t%s\n' "$now" "$old_pid" "$expected_pid" "$old_start" "$new_start" >>"$evidence_root/restarts.tsv"
    next_restart=$((next_restart+restart_interval)); sample
  fi
done
expected_restarts=$((duration/restart_interval)); ((planned_restarts>=expected_restarts)) || fail_run "planned restart count is incomplete"
