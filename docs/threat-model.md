...
| T9 | Disk exhaustion or deletion races | Known-size cache admissions reserve against a shared `max_bytes` budget and filesystem headroom; high/low-watermark GC removes global LRU complete objects; startup reconstructs committed usage; age GC ignores active temps; store locking protects local commits | Unknown-length requests bypass caching, while in-flight temps, malformed/orphaned bytes, private PyPI files, metadata, and filesystem overhead are not all evictable under the logical budget; it is not a hard filesystem quota. The deterministic pressure/restart test covers accounting and eviction, but the real seven-day run and deployment headroom evidence remain pending |
